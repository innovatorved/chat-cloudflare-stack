import { ArrowLeft, ChatText, Gear, Plus } from "@phosphor-icons/react";
...
export default function ChatsView() {
  ...
  const handleAuthSuccess = async (userHasAiKey: boolean) => {
    setAuthenticated(true);
    setHasAiKey(userHasAiKey);
    setChats([]);
    setError(null);
    setSelectedChat(null);
    setCreatingChat(false);
  ...
  useEffect(() => {
    if (authenticated && hasAiKey) {
      fetchChats();
    }
  }, [authenticated, hasAiKey, fetchChats]);

  // Auth screens
  if (!authenticated) {
    if (authView === "signup")
      return <SignUp onSignUp={handleAuthSuccess} setView={setAuthView} />;
    return <Login onLogin={handleAuthSuccess} setView={setAuthView} />;
  }

  if (!hasAiKey) {
    return <ApiKeySetup onSaved={handleApiKeySaved} onLogout={handleLogout} />;
  }
